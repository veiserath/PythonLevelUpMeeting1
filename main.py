...
def func(request: Request, token: str = Query(None), format: Optional[str] = None):
    #if [i for i in token if i in app.tokens_login_token] == []:
    if token not in app.tokens_login_token:
        raise HTTPException(status_code=401, detail="Unathorised")
    print(f'app.tokens_login_token: {app.tokens_login_token}')
    print(f'token: {token}')
...
@app.get('/logged_out')
def func(format: Optional[str] = None):
    if format == 'json':
